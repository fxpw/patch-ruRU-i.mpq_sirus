...
local settings = {
	...
	cacheIndex = 1,					-- Per layout action, the index of where processing is in the cachedOrderList
...
function ObjectiveTrackerModuleMixin:OnLoad()
	...
	if self.OnHeaderFilterButtonClick then
		self.Header.FilterButton:Show()
		self.Header.FilterButton:SetScript("OnClick", GenerateClosure(self.OnHeaderFilterButtonClick, self));
	end
end
...
function ObjectiveTrackerModuleMixin:SetContainer(container)
	...
	self:SetParent(container.ScrollFrame.ScrollChild);
	self:ClearAllPoints();
...
function ObjectiveTrackerModuleMixin:Update(availableHeight, dirtyUpdate)
	...
	if not self.hasContents and self.collapseType and self:CanUpdate() then
...
function ObjectiveTrackerModuleMixin:AcquireFrame(template, templateType)
	local frame, isNew = ObjectiveTrackerManager:AcquireFrame(self, template, templateType);
...
function ObjectiveTrackerModuleMixin:SetCollapsedForce(collapsed)
	self.isCollapsed = collapsed
	self.ContentsFrame:SetShown(not collapsed)
	-- update the header
	self.Header:SetCollapsed(collapsed)
...
function ObjectiveTrackerModuleMixin:GetRightEdgeFrame(settings, identifier)
	...
	if not frame then
		frame = self:AcquireFrame(settings.template, settings.templateType);
		frame:Show();
...
function ObjectiveTrackerModuleHeaderMixin:OnLoad()
	self.Background:SetAtlas("UI-QuestTracker-Secondary-Objective-Header", true)
	self.Shine:SetAtlas("UI-QuestTracker-OBJFX-Shine", true) -- Scale 0.95
	self.Glow:SetAtlas("UI-QuestTracker-OBJFX-BarGlow", true)
...
function ObjectiveTrackerModuleHeaderMixin:SetCollapsed(collapsed)
	...
	local module = self:GetParent();
	if module.collapseType then
		SetCVarBitfield("objectiveTrackerCollapsedState", module.collapseType, collapsed)
	end
end
