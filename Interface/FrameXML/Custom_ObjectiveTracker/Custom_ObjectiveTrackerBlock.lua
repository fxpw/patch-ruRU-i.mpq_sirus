...
function ObjectiveTrackerBlockMixin:AddObjective(objectiveKey, text, template, useFullHeight, dashStyle, colorStyle, adjustForNoText, overrideHeight)
	...
	if line.Dash then
		...
		if line.dashStyle ~= dashStyle then
			...
			elseif dashStyle == OBJECTIVE_DASH_STYLE_HIDE or dashStyle == OBJECTIVE_DASH_STYLE_NONE then
				line.Dash:Hide();
				line.Dash:SetText(QUEST_DASH);
			elseif dashStyle == OBJECTIVE_DASH_STYLE_ICON then
				line.Dash:Hide();
				line.Dash:SetText(nil);
			else
				assert(false, "Invalid dash style: " .. tostring(dashStyle));
			end
