...
local permittedAPI =
{
	[AsyncCallbackAPIType.ASYNC_QUEST] = { event = "QUEST_DATA_LOAD_RESULT", accessor = RequestLoadQuestByID },
	[AsyncCallbackAPIType.ASYNC_ITEM] = { event = "ITEM_DATA_LOAD_RESULT", accessor = C_Item.RequestLoadItemDataByID },
--	[AsyncCallbackAPIType.ASYNC_CREATURE] = { event = "CREATURE_DATA_LOAD_RESULT", accessor = RequestLoadCreatureByID },
};
	...
	if callbacks then
		...
		for i, callback in ipairs(callbacks) do
			if callback ~= CANCELED_SENTINEL then
				local success, results = pcall(callback);
				if not success then
...
--CreatureEventListener = CreateListener(AsyncCallbackAPIType.ASYNC_CREATURE);
